...
straight_cuts = {};
cut = 1;
files = dir('../data/preprocessed_transformed_data/april/test_depth/*_cut_*.json');
for file = files'    
    ...
    if strcmp(surfname, 'apple_06') || strcmp(surfname, 'orange_02')
        continue
    end
            ...
            if message.model.depth <= 0
                ...
                for body=1:length(message.filtered.bodies)
                    if strcmp(message.filtered.bodies{body}.frame,'robot') && strcmp(message.filtered.bodies{body}.name,'ee')
                        ...
                        ee_vel = [ee_vel; message.filtered.bodies{body}.twist.linear'];
                    end
