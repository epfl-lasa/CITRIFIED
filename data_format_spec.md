...
The low frequency message is generated by the classifier.
```json
"esn": {
  "input": {...},
  "output": {...}
...
```
- `esn`
  - `input` - the data that is given to the classifier (matrix as time-window of signals)
  - `output` - the output structure (label probabilities)
