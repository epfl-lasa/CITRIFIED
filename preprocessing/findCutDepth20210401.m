...
files = dir('../data/raw_data/april/test/*_cut_*.json');
for file = files'
    ...
    if strcmp(surfname, 'apple_05') && strcmp(trialname{1},'cut_10')
        continue
    end
    if strcmp(surfname, 'orange_01') && strcmp(trialname{1},'cut_01')
        continue
    end
    
    % load the associated surface data and fit surface
    surfFile = dir(['../data/raw_data/april/test/20210401_' surfname '_surface*']);
    if numel(surfFile) ~= 1
        continue
    end
        ...
        if strcmp(message.control.phase,'cut')
            ...
            for body=1:length(message.filtered.bodies)
                if strcmp(message.filtered.bodies{body}.frame,'robot') && strcmp(message.filtered.bodies{body}.name,'ee')
                    ee_pos = [ee_pos; message.filtered.bodies{body}.pose.position'];
                end
...
function surfname = extractSurfaceName(filename)
    surfname = regexp(filename, '20210401_(.*?_\d\d)_.*', 'tokens');
    if ~isempty(surfname)
