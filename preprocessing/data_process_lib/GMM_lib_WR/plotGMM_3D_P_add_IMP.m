function plotGMM_3D_P_add_IMP(Mu, Sigma, color, display_mode);
%
...
ellpsiod_magnitude=0.8; %3.0  adjust the ellpsiod of 1 2
ellpsiod_magnitude_1=0.8; %3  adjust the ellpsiod of 3
...
save GMR_trj.mat Data_trj
  ...
  for j=1:nbData
      h = plot_gaussian_ellipsoid_P_add_IMP( Mu(:,j)', Sigma(:,:,j)',0.8); %last one is for the magnitude of ellipsoid
  end
  
    elseif display_mode==6%another 5 for imp ellispiod
  for j=1:nbData
      h = plot_gaussian_ellipsoid_P_add_IMP( Mu(:,j)', Sigma(:,:,j)',0.006); %last one is for the magnitude of ellipsoid
  end
