...
%%%%%%%%% data we see on C5
    spectralRadius_0_1_kind_best=5;
        ...
        for TW_kind=TW_kind_best
            ...
            for trials_num=1:size(test_cell,1)
                ...
                if learning_method==1
                    ...
                    %%% get data
                    for TW_num=1:size(test_cell{trials_num},1)
                        testInputSequence{TW_num}=test_cell{trials_num}(TW_num,1:end-1);
                        testOutputSequence{TW_num}=test_cell{trials_num}(TW_num,end);
                    end

                    %%% generate feature matrix for SVM learning (no need for time sequency)
                    SignalFeatures_all_exp=[];
                    for time=1:size(test_cell{trials_num},1)
                         SignalFeatures_all_exp=[SignalFeatures_all_exp; testInputSequence{time}];
                    end
                    ...
                    for TW_num=1:length(testInputSequence)
                        predictedTestOutput{TW_num} = 0;
                        tic
                        predictedTestOutput{TW_num}= predict(SVMModel,testInputSequence{TW_num});
                    ...
                    real_label=cell2mat(testOutputSequence);
                    label=cell2mat(predictedTestOutput);
