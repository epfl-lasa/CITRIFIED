function [Mu Sigma x_first x_last imp imp_first imp_last] = gmm_gmr_learn_imp_from_pos_out_GMR(demos)
%
...
%% Plot of the data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('position',[10,10,1000,800],'name','data_GMM-demo1');
%plot 1D
for n=1:nbVar-1
...
view(-29,15); set(gca,'proj','perspective'); grid on;
  grid on;  axis tight;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%plot 1D
figure('position',[10,10,1000,800],'name','GMR-demo1');
for n=1:3
...
%axis([min(Data(2,:))-0.01 max(Data(2,:))+0.01 min(Data(3,:))-0.01 max(Data(3,:))+0.01 min(Data(4,:))-0.01 max(Data(4,:))+0.01]);
xlabel('x_1','fontsize',16); ylabel('x_2','fontsize',16); zlabel('x_3','fontsize',16);
view(-21,15);axis equal;%axis tight;
...
imp=expData([5,6,7],:);
imp_first=expData([5,6,7],1);
imp_last=expData([5,6,7],end);

% %output the GMM data
% Mu=Mu([2,3,4],:);
% Sigma=Sigma([2,3,4],[2,3,4],:);
% x_first=expData([2,3,4],1);
% x_last=expData([2,3,4],end);
% imp=Mu([5,6,7],:);
% imp_Sigma=Sigma([5,6,7],[5,6,7],:);
