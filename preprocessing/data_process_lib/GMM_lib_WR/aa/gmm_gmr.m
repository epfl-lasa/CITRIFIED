%% ����GMM-GMR֮�󽫵õ��Ĺ켣�浽�ļ���data��
function [Mu Sigma x_first x_last] = gmm_gmr(demos)
%
...
Data=demos;
% load('data/data3800.mat'); %load 'Data'data11ÿ����ݶ���3800�����飬��data111���һ��4200
nbVar = size(Data,1);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
expData(1,:) = linspace(min(Data(1,:)), max(Data(1,:)), 6100);%%%100��Ϊ3800
...
plot3(Data(2,:), Data(3,:), Data(4,:), 'x', 'markerSize', 4, 'color', [.3 .3 .3]);
...
%plot 1D
for n=1:nbVar-1
  subplot(3*(nbVar-1),2,6+(n-1)*2+1); hold on;
  plotGMM_3D(Mu([1,n+1],:), Sigma([1,n+1],[1,n+1],:), [0 .8 0], 1);
  axis([min(Data(1,:)) max(Data(1,:)) min(Data(n+1,:))-0.01 max(Data(n+1,:))+0.01]);
  xlabel('t','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
end
%plot 3D
subplot(3*(nbVar-1),2,6+[2:2:2*(nbVar-1)]); hold on;
plotGMM_3D(Mu([2,3,4],:), Sigma([2,3,4],[2,3,4],:), [0 .8 0], 5);
...
%plot 1D
for n=1:nbVar-1
  subplot(3*(nbVar-1),2,12+(n-1)*2+1); hold on;
  plotGMM_3D(expData([1,n+1],:), expSigma(n,n,:), [0 0 .8], 3);
  axis([min(Data(1,:)) max(Data(1,:)) min(Data(n+1,:))-0.01 max(Data(n+1,:))+0.01]);
  xlabel('t','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
end
%plot 3D
subplot(3*(nbVar-1),2,12+[2:2:2*(nbVar-1)]); hold on;
plotGMM_3D(expData([2,3,4],:), expSigma([1,2,3],[1,2,3],:), [0 0 .8], 4);
axis([min(Data(2,:))-0.01 max(Data(2,:))+0.01 min(Data(3,:))-0.01 max(Data(3,:))+0.01 min(Data(4,:))-0.01 max(Data(4,:))+0.01]);
xlabel('x_1','fontsize',16); ylabel('x_2','fontsize',16); zlabel('x_3','fontsize',16);
view(-85,53)
