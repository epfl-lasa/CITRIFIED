...
LagsDS::LagsDS(double frequency,
               int k,
               int m,
...
bool LagsDS::initializeDS() {
  ...
  CCDyn_filter_ = std::make_unique<fast_gmm::CDDynamics>(m_, dt_, Wn_);
  ...
  Eigen::VectorXd initial = Eigen::VectorXd::Zero(m_);
  CCDyn_filter_->SetState(initial);
  CCDyn_filter_->SetTarget(initial);
  return true;
}

Eigen::VectorXd LagsDS::computeDesiredVelocity(const Eigen::VectorXd& input) {
  mutex_.lock();
  ...
  CCDyn_filter_->SetTarget(desired_velocity);
  CCDyn_filter_->Update();
  CCDyn_filter_->GetState(desired_velocity_filtered);
