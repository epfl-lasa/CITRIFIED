...
if do_grid==1
    ...
    spectralRadius_grid_best = [0.1:0.1:1]; 
    numInternalUnits_grid_best= [100:100:1000];
%     TW_grid=[50:50:250]*0.001;
    TW_grid_best=[100]*0.001;
    
%     spectralRadius_grid_best = [0.1 0.2 0.3]; 
%     numInternalUnits_grid_best= [800 900 1000];
%     TW_grid_best=[100]*0.001;
%     
    spectralRadius_grid_best = 0.4; 
    numInternalUnits_grid_best= 300;
    TW_grid_best=[100]*0.001;
...
plot_test_train_data=0;

plot_time_serise_label_predict=0;
...
for spectralRadius_0_1=spectralRadius_grid_best 
    for numInternalUnits=numInternalUnits_grid_best 
        for TW=TW_grid_best 
            ...
            %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% load learning model
            load([path_of_load_model learning_method_name '_TW_' num2str(TW) '_InterUnit_' num2str(numInternalUnits) '_spectralRadius_' num2str(spectralRadius_0_1) '_learning_4_tissue.mat']);
            ...
            %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            %% check the data if it is too similar
            %% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

            if plot_test_train_data==1
                figure
                train_together=[];
                subplot(411)
                for i=1:size(train_cell,1)
                    for j=1:size(train_cell{i},2)
                        data_num=length(train_cell{i}{j}(:,1));
                        plot([(j-1)*data_num+1:(j)*data_num],train_cell{i}{j}(:,1),'b');hold on; grid on;
                    end
                end
                for i=1:size(test_cell,1)
                    for j=1:size(test_cell{i},2)
                        data_num=length(test_cell{i}{j}(:,1));
                        plot([(j-1)*data_num+1:(j)*data_num],test_cell{i}{j}(:,1),'r');hold on; grid on;
                    end
                end
                ylabel('depth');
                subplot(412)
                for i=1:size(train_cell,1)
                    for j=1:size(train_cell{i},2)
                        data_num=length(train_cell{i}{j}(:,1));
                        plot([(j-1)*data_num+1:(j)*data_num],train_cell{i}{j}(:,2),'b');hold on; grid on;
                    end
                end
                for i=1:size(test_cell,1)
                    for j=1:size(test_cell{i},2)
                        data_num=length(test_cell{i}{j}(:,1));
                        plot([(j-1)*data_num+1:(j)*data_num],test_cell{i}{j}(:,2),'r');hold on; grid on;
                    end
                end
                ylabel('velocity');
                subplot(413)
                for i=1:size(train_cell,1)
                    for j=1:size(train_cell{i},2)
                        data_num=length(train_cell{i}{j}(:,1));
                        plot([(j-1)*data_num+1:(j)*data_num],train_cell{i}{j}(:,3),'b');hold on; grid on;
                    end
                end
                for i=1:size(test_cell,1)
                    for j=1:size(test_cell{i},2)
                        data_num=length(test_cell{i}{j}(:,1));
                        plot([(j-1)*data_num+1:(j)*data_num],test_cell{i}{j}(:,3),'r');hold on; grid on;
                    end
                end
                ylabel('force');
                subplot(414)
...
%                 end
                ...
                if plot_time_serise_label_predict==1
                    figure
                    plot(time_for_plot,plot_input_time_serise(:,1)*1);hold on;
                    plot(time_for_plot,plot_input_time_serise(:,2)*1);hold on;
                    plot(time_for_plot,plot_input_time_serise(:,3)*1);hold on;
                    plot(time_for_plot,plot_input_time_serise(:,4)*1);hold on;
    ...
    %                 plot([1:length(real_out1)],real_out1(:),'-+');hold on;
                    legend('depth','velocity','force','force deritive','real-label','predict-label','Location','best');%,'MV-label','MV-real-out'
...
%             yticks([2 4 6]);yticklabels({'apple','banana','orange'});
            title(['classify confusion matrix for all fruit' num2str(spectralRadius_0_1) num2str(numInternalUnits) num2str(TW)])
            
            %% get finial success rate
            A=[spectralRadius_0_1 numInternalUnits TW];
            fprintf('the spectralRadius_0_1 %f, numInternalUnits %f, TW %f,',A)
...
file_mat=[path_of_load_model 'ESN_march30_' num2str(spectralRadius_0_1) num2str(numInternalUnits) num2str(TW) '.mat'];
save(file_mat,'trainedEsn');

file=[path_of_load_model 'ESN_march30_' num2str(spectralRadius_0_1) num2str(numInternalUnits) num2str(TW) '.txt'];
nfp=nForgetPoints;   %%% nForgetPoints
a=writeESNtofile(trainedEsn, file, nfp);
if a
    !echo Write ESN to txt success!
else
    !echo Write ESN to txt Error!
end

file=[path_of_load_model 'ESN_march30_' num2str(spectralRadius_0_1) num2str(numInternalUnits) num2str(TW) '.yaml'];
b=writeESNtoYAML(trainedEsn, file, nfp, class_label);

if b
    !echo Write ESN to yaml success!
else
    !echo Write ESN to yaml Error!
end
