function [Mu,Sigma,Priors,expData,expSigma,data_mean,data_range] = gmm_gmr_F2vI(demos,input_dim,output_dim,verfine_data,nbStates,normalize,ellpsiod_magnitude_GMM,ellpsiod_magnitude_GMM_3D,ellpsiod_magnitude_GMR,ellpsiod_magnitude_GMR_3D)
%
...
for n=1:input_dim
  ...
  plot(Data(n,:), Data(n+input_dim,:), '*', 'markerSize', 1);%, 'color', [.3 .3 .3],'lineWidth', 3
  axis([min(Data(n,:)) max(Data(n,:)) min(Data(n+input_dim,:))-0.01 max(Data(n+input_dim,:))+0.01]);
  xlabel('p','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
end

for n=1:input_dim
  subplot(1,3,n); hold on;
  plotGMM_3D_P_add_IMP_adj_size([Mu(n,:);Mu(n+input_dim,:)], Sigma([1,n],[1,n],:), [0 .8 0], 1,ellpsiod_magnitude_GMM,ellpsiod_magnitude_GMM_3D);
  axis([min(Data(n,:)) max(Data(n,:)) min(Data(n+input_dim,:))-0.01 max(Data(n+input_dim,:))+0.01]);
  xlabel('p','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
