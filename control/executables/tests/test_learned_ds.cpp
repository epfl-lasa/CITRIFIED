...
#include "motion_generators/LagsDS.h"
...
//
//  motion_generator::CoupledDS ds(frequency, k_gmm, dim, priors, mu, sigma, muScale, sigmaScale, attractor);
//
//  if (!ds.init()) {
//    return -1;
//  }
...
//  auto twist = ds.computeDesiredVelocity(eeInRobot.get_position());
...
int main(int argc, char** argv) {
  ...
  motion_generator::CoupledDS ds(frequency, k_gmm, dim, priors, mu, sigma, muScale, sigmaScale, attractor_vec);
...
//      ds(frequency, K_gmm, M_gmm, Priors, Mu, Sigma, A_g, att_g, A_l, A_d, att_l, w_l, b_l, scale, b_g, gpr_path);
  ...
  state_representation::CartesianPose
      attractor = state_representation::CartesianPose("attractor", Eigen::Quaterniond(0, 1, 0, 0), "franka");
