function [Mu,Sigma,Priors,expData,expSigma,data_mean,data_std,MSE] = gmm_gmr_D1vD1_CV(demos,input_dim,output_dim,verfine_data,nbStates,normalize_or_not,ellpsiod_magnitude_GMM,ellpsiod_magnitude_GMM_3D,ellpsiod_magnitude_GMR,ellpsiod_magnitude_GMR_3D,plot_on)
%
...
if normalize_or_not==1
    ...
    mean_input=mean(Data(1,:),2);
    std_input=std(Data(1,:)');
    mean_output=mean(Data(2,:),2);
    std_output=std(Data(2,:)');
    
    Data1(1,:)=(Data(1,:)-mean_input(1))/std_input(1);
%     Data1(2,:)=(Data(2,:)-mean_input(2))/std_input(2);
%     Data1(3,:)=(Data(3,:)-mean_input(3))/std_input(3);
    Data1(2,:)=(Data(2,:)-mean_output(1))/std_output(1);
%     Data1(5,:)=(Data(5,:)-mean_output(2))/std_output(2);
%     Data1(6,:)=(Data(6,:)-mean_output(3))/std_output(3);
    
    Data2(1,:)=(Data(1,:)-mean_input)/sqrt(std_input(1)^2);
    Data2(2,:)=(Data(2,:)-mean_output)/sqrt(std_output(1)^2);
...
%     end
    ...
    Data=Data2;
    verfine_data(2,:)=(verfine_data(2,:)-mean_input)/sqrt(std_input(1)^2);
    verfine_data(3,:)=(verfine_data(3,:)-mean_output)/sqrt(std_output(1)^2);
    clear Data1 Data2
...
for n=1:input_dim
  ...
  axis([min(Data(n,:)) max(Data(n,:)) min(Data(input_dim+n,:))-0.002 max(Data(input_dim+n,:))+0.002]);
...
for n=1:input_dim+output_dim
    for m=1:input_dim+output_dim
      subplot(6,6,(n-1)*6+m); hold on; grid on;
      plot(Data(n,:), Data(m,:), 'b*', 'markerSize', 1);%, 'color', [.3 .3 .3],'lineWidth', 3
    end
end
for n=1:input_dim+output_dim
    for m=1:input_dim+output_dim 
      subplot(6,6,(n-1)*6+m); hold on;
