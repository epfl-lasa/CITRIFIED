...
%% data paths
raw_data_path = fullfile('..','..','data','raw_data','april','scalpel_test');
...
%     end
% end
% save(fullfile(raw_data_path,'timewindows_20210427.mat'),'all_timewindows');
load(fullfile(raw_data_path,'timewindows_20210427.mat'));
...
for nb_class = 1:size(all_timewindows,1)
    ...
    for insertion = 1:length(data)
        for tw = 1:3
            input = data{insertion}{tw}.input;
            plot3(input.depth, input.velocity_z, input.force_derivative_z, colors{nb_class});
        end
    end
end
xlabel('depth')
