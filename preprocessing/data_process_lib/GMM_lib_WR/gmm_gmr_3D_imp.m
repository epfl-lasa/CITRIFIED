function [Mu Sigma Priors] = gmm_gmr_3D_imp(demos)
%
...
%% Plot of the data
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure('position',[10,10,1000,800],'name','data_GMM-impedance');
%plot 1D
for n=1:nbVar-1
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%plot 1D
figure('position',[10,10,1000,800],'name','GMR-impedance');
for n=1:nbVar-1
  subplot(6,1,n); hold on;
  plotGMM_3D_P_add_IMP(expData([1,n+1],:), expSigma(n,n,:), [0 0 .8], 3);
  axis([min(Data(1,:)) max(Data(1,:)) min(Data(n+1,:))-0.01 max(Data(n+1,:))+0.01]);
  xlabel('t','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
end
%plot 3D
subplot(6,1,[4 5 6]); hold on;
