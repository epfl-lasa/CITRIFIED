function [Mu Sigma Priors] = gmm_gmr_pos2force_base_time(demos,nbStates,ellpsiod_magnitude_GMM,ellpsiod_magnitude_GMM_3D,ellpsiod_magnitude_GMR,ellpsiod_magnitude_GMR_3D)
%
...
for n=1:nbVar-1
  subplot(6,2,(n-1)*2+1); hold on;
  plotGMM_3D_P_add_IMP_adj_size(Mu([1,n+1],:), Sigma([1,n+1],[1,n+1],:), [0 .8 0], 1,ellpsiod_magnitude_GMM,ellpsiod_magnitude_GMM_3D);
  axis([min(Data(1,:)) max(Data(1,:)) min(Data(n+1,:))-0.01 max(Data(n+1,:))+0.01]);
  xlabel('p','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
subplot(6,2,[2:2:2*(nbVar-1)]); hold on;
plotGMM_3D_P_add_IMP_adj_size(Mu([2,3,4],:), Sigma([5,6,7],[5,6,7],:), [0 .8 0], 5,ellpsiod_magnitude_GMM,ellpsiod_magnitude_GMM_3D);
...
for n=1:3
  ...
  plotGMM_3D_P_add_IMP_adj_size(expData([1,n+1],:), expSigma(n,n,:), [0 0 .8], 3,ellpsiod_magnitude_GMR,ellpsiod_magnitude_GMR_3D);
  axis([min(Data(1,:)) max(Data(1,:)) min(Data(n+1,:))-0.01 max(Data(n+1,:))+0.01]);
  xlabel('p','fontsize',16); ylabel(['x_' num2str(n)],'fontsize',16);
end
for n=4:6
...
plotGMM_3D_P_add_IMP_adj_size(expData([2,3,4],:), expSigma([1,2,3],[1,2,3],:), [0 0 .8], 4,ellpsiod_magnitude_GMR,ellpsiod_magnitude_GMR_3D);
axis([min(Data(2,:))-0.01 max(Data(2,:))+0.01 min(Data(3,:))-0.01 max(Data(3,:))+0.01 min(Data(4,:))-0.01 max(Data(4,:))+0.01]);
xlabel('x_1','fontsize',16); ylabel('x_2','fontsize',16); zlabel('x_3','fontsize',16);
...
for num_ellispiod_imp=1:7  
    ellispiod_imp(:,:,num_ellispiod_imp)=[expData(5,number_windows*(num_ellispiod_imp-1)+1) 0 0; 0 expData(6,number_windows*(num_ellispiod_imp-1)+1) 0; 0 0 expData(7,number_windows*(num_ellispiod_imp-1)+1)];
end

plotGMM_3D_P_add_IMP_adj_size(expData([2,3,4],1:number_windows:end), expSigma([4,5,6],[4,5,6],1:number_windows:end), [0 0 .8], 5,ellpsiod_magnitude_GMR,ellpsiod_magnitude_GMR_3D);
