function [Data] = cut2d_by_imp(demos,time,imp)
%
...
%     for g=1:length(tmp)
%         if tmp(g)<=200.0 & tmp(g+1)>=200.0
%         ind_first=g;
%         else if tmp(g)>=200.0 & tmp(g+1)<=200.0
%                 ind_last= g;
%                 break
%             end
%         end
...
%         ind_first=find(tmp,1,'first')
%         ind_last= find(tmp,1,'last')
        
    H=[ind_first ind_last-ind_first length(demos{i})-ind_last]
