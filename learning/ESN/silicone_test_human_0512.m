...
% dir_content = struct2cell(dir([raw_data_path '/*0512*silicon_04*.json']'));
% insertion = 1;
...
% end
% save(fullfile(raw_data_path,'silicon_human_0512.mat'),'insertions');
load(fullfile(raw_data_path,'silicon_human_0512.mat'));

data = insertions{1};
data(find(-data(:,2)>0.008,1):end,:) = [];
insertions{1} = data;

data = insertions{2};
data(find(-data(:,2)>0.01,1):end,:) = [];
insertions{2} = data;

data = insertions{3};
data(find(-data(:,2)>0.018,1):end,:) = [];
insertions{3} = data;

data = insertions{7};
data(find(-data(:,2)>0.014,1):end,:) = [];
insertions{7} = data;
...
%%
data = insertions{1};
data(find(data(:,1)>4.6,1):end,:) = [];
data(:,end+1) = data(:,1) > 4.3;
data(data(:,1)<2.5,:) = [];
labeled{1} = data;

data = insertions{2};
data(find(data(:,1)>3.3,1):end,:) = [];
data(:,end+1) = data(:,1) > 3.0;
data(data(:,1)<1,:) = [];
labeled{2} = data;

data = insertions{3};
data(find(data(:,1)>4.15,1):end,:) = [];
data(:,end+1) = data(:,1) > 3.8;
data(data(:,1)<2,:) = [];
labeled{3} = data;

data = insertions{7};
data(find(data(:,1)>3.25,1):end,:) = [];
data(:,end+1) = data(:,1) > 2.8;
data(data(:,1)<1,:) = [];
labeled{4} = data;

save(fullfile(raw_data_path,'silicon_human_0512_labeled.mat'),'labeled');
%%
figure;
x_col = 1;
ax1 = subplot(4,1,1);
